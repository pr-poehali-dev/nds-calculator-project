...
const OKVEDCalculator = () => {
  ...
  const getUSNRate2025 = (revenue: number, employees: number): number => {
    ...
    if (revenue >= 250) return 18;
    if (revenue >= 60) return 11;
    return 6;
  };
  ...
  const psnWithPatent2025 = calculateTax(revenueInRubles, 0);
  const psnWithPatent2026 = calculateTax(revenueInRubles, 0);
  const psnWithoutPatent2025 = calculateTax(revenueInRubles, 6);
  const psnWithoutPatent2026 = calculateTax(revenueInRubles, 6);
          ...
          {showResults && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/95 backdrop-blur-sm p-4 sm:p-8 animate-in fade-in duration-300">
              <button
                ...
                className="fixed top-4 right-4 sm:top-8 sm:right-8 w-12 h-12 rounded-full bg-slate-800/80 hover:bg-slate-700 border border-slate-700/50 flex items-center justify-center text-slate-400 hover:text-white transition-all duration-200 hover:scale-110 z-[100] shadow-2xl"
              >
                <Icon name="X" size={24} />
              </button>
